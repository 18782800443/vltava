...
public interface MockManageMapper extends MyMapper<MockManage> {
    @Select("select count(1) from mock_manage where app_id=#{appId} and yn=1 and class_name=#{className} and method_name=#{methodName}")
    Integer exist(@Param("appId") Integer appId, @Param("className") String className, @Param("methodName") String methodName);

    @Select("select id from mock_manage where mock_key=#{mockKey}")
    ...
    @Select("select id, app_id, task_status, mock_key, version from mock_manage where id=${id}")
    MockManage getSimpleInfoById(@Param("id") Integer id);

    @Select("select * from mock_manage where app_id=#{appId} and yn=1")
    List<MockManage> selectByAppIdAndYn(@Param("appId") Integer appId);

    @Select("select version from mock_manage where id=#{id}")
    Integer selectVersionById(@Param("id") Integer id);

    @Update("update mock_manage set connect=${status} where app_id=${appId}")
    ...
    @Select("select * from mock_manage as a where a.id in (select id from app_manage where system_unique_name=${systemUniqueName} " +
